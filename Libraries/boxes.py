...
def simple_box(width, length, height, wall_thickness = 2, floor_thickness = 2, fillet = 0):
    ...
    cutter = cq.Workplane(origin=(0, 0, floor_thickness)).box(width - 2 * wall_thickness, length - 2 * wall_thickness, height)
    if fillet:
...
def hexagonal_floor_holes_box(width, length, height, wall_thickness = 2, floor_thickness = 2, fillet = 0):
    ...
    for x in range(-max_iterations, max_iterations + 1):
        for y in range(-max_iterations, max_iterations + 1):
            v = x * v1 + y * v2
            if abs(v.x) <= width / 2 - wall_thickness - fillet - hole_diameter / 2 and abs(v.y) <= length / 2 - wall_thickness - fillet - hole_diameter / 2:
                centers.append(v)

    return box.faces("<Y").workplane(centerOption="CenterOfMass").pushPoints([(0, 3)]).hole(10).faces("<Z").workplane(centerOption="CenterOfMass").pushPoints(centers).polygon(6, hole_diameter).cutThruAll()


a = hexagonal_floor_holes_box(76, 131, 20, 1.5, 2, 5).translate((76 / 2, 131 / 2, 20 / 2))

cq.exporters.export(a, "box.stl")